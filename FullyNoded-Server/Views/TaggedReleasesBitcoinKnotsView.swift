...
struct TaggedReleasesBitcoinKnotsView: View {
    ...
    @State private var versionNumberInt: String?
    ...
    var body: some View {
        if !installComplete {
            ...
            if let author = taggedRelease.author, let login = author.login, let tagName = taggedRelease.tagName {
                ...
                if let versionNumberInt = versionNumberInt {
                    Text(verbatim: "Downloading from https://bitcoinknots.org/files/\(versionNumberInt)x/\(tagName)-\(arch)-apple-darwin.tar.gz")
                        .padding(.leading)
                        .frame(maxWidth: .infinity, alignment: .leading)
                }
                
                HStack() {
                    let url = "https://github.com/bitcoinknots/bitcoin/blob/\(tagName)/doc/release-notes.md"
                ...
                HStack() {
                    if isAnimating {
                        ProgressView()
                            .scaleEffect(0.5)
                        Text("Installing Bitcoin Knots \(tagName)...")
                    } else {
                        Button {
                            isAnimating = true
...
//                                }
    ...
    private func saveEnvVaules(version: String) {
        DataManager.deleteAllData(entityName: .bitcoinKnotsEnv) { deleted in
